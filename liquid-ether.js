/* Liquid Ether — adapted for vanilla JS from reactbits.dev's LiquidEther
   React component (DavidHDev/react-bits, MIT license). All simulation
   logic (fluid advection/divergence/pressure shaders) is unchanged;
   only the React mounting layer (hooks/refs) was replaced with a plain
   init function so it can run without a build step.

   Requires Three.js (global `THREE`) loaded before this script.

   Usage:
     const handle = initLiquidEther(document.getElementById('liquid-ether-bg'), {
       colors: ['#c9a84c', '#7a1f1f', '#3a2508']
     });
     // handle.dispose() to tear down if ever needed
*/
